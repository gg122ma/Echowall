...
(function () {
  ...
  function readClient() {
    // Before auth restoration succeeds (or if it fails because the stored
    // session is stale), reads use a session-free anon client. Once a valid
    // user is published, auth-change handlers refetch with getClient() so
    // per-user fields such as can_manage_question remain available.
    if (window.SupabaseAuthProvider?.getCurrentUser?.()) return window.CommunitySupabaseClient.getClient();
    return window.CommunitySupabaseClient.getPublicClient();
  }
