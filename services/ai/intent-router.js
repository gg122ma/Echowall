(function () {
  ...
  const REQUEST_SIGNAL = /\b(?:need|needs|needed|want|wants|wanted|require|requires|required|where|somewhere|place|any|can|could|may|how|what about|nak|perlu|hendak|mana|ada|boleh)\b|需要|要|可以|能|有|哪里|哪儿|去哪/u;
  ...
  function analyzeServiceFrame(message) {
    ...
    if (!text) return Object.freeze({ state: "NONE", frames: Object.freeze([]), evidence: Object.freeze([]) });
    ...
    const relation = evidence(/\b(?:to|need|needs|needed|require|requires|required)\b|需要/u, text, "RELATION");
    ...
    const frameSignal = request || relation || imperative;
    const comparisonSignal = /\b(?:which|compare)\b[^.!?]*\b(?:and|versus|vs)\b/u.test(text);
    const frames = [];
    const add = (kind, action, object, parts, inferred = false) => {
      if (!frames.some(frame => frame.kind === kind)) frames.push(Object.freeze({ kind, action, object, inferred, evidence: Object.freeze(parts.filter(Boolean)) }));
    };
    if (frameSignal && concepts.print && concepts.document) add("PRINT_DOCUMENT", "PRINT", "DOCUMENT", [request, relation, concepts.print, concepts.document]);
    if (comparisonSignal && concepts.print) add("PRINT_DOCUMENT", "PRINT", "DOCUMENT", [request, concepts.print], true);
    if (frameSignal && concepts.wash && concepts.clothes) add("WASH_CLOTHES", "WASH", "CLOTHES", [request, relation, concepts.wash, concepts.clothes]);
    if (frameSignal && concepts.iron && concepts.clothes) add("IRON_CLOTHES", "IRON", "CLOTHES", [request, relation, concepts.iron, concepts.clothes]);
    if (frameSignal && concepts.borrow && concepts.bicycle) add("BORROW_BICYCLE", "BORROW", "BICYCLE", [request, relation, concepts.borrow, concepts.bicycle]);
    if (frameSignal && concepts.borrow && concepts.sports && concepts.equipment) add("BORROW_SPORTS_EQUIPMENT", "BORROW", "SPORTS_EQUIPMENT", [request, relation, concepts.borrow, concepts.sports, concepts.equipment]);
    ...
    return Object.freeze({ state: frames.length ? "COMPLETE" : "NONE", frames: Object.freeze(frames), evidence: Object.freeze(Object.values(concepts).filter(Boolean)) });
  }
