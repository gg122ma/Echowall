(function () {
  ...
  function specialText(plan, language) {
    if (plan.content.primary === "DINING_DISCOVERY") return language === "ms" ? "KMK mempunyai beberapa tempat makan, termasuk Cafe A, Cafe B, Cafe C dan Cafe Admin. Yang mana satu anda mahu saya tunjukkan pada Echo Map?" : language === "zh" ? "KMK 有多个用餐地点，包括 Cafe A、Cafe B、Cafe C 和 Cafe Admin。你想让我在 Echo Map 上显示哪一个？" : "KMK has several dining locations, including Cafe A, Cafe B, Cafe C, and Cafe Admin. Which one would you like me to show on Echo Map?";
    if (plan.content.primary === "SPORTS_DISCOVERY") return language === "ms" ? "Rekod yang diluluskan menyokong Astaka dan Basketball Court sebagai kemudahan sukan. Stor Sukan juga disokong untuk pinjaman peralatan sukan dan basikal, tetapi ketersediaan semasa serta sasaran tepat Echo Map belum disahkan. Nyatakan tempat tertentu untuk maklumat lanjut." : language === "zh" ? "当前获批资料支持 Astaka 和 Basketball Court 作为体育设施。Stor Sukan 也提供体育器材和自行车借用信息，但当前可用情况及准确的 Echo Map 目标尚未核实。请说出具体地点以获取详情。" : "Current approved records support Astaka and the Basketball Court as sports facilities. Stor Sukan is also supported for sports-equipment and bicycle borrowing, but its current availability and exact Echo Map target are not verified. Name a specific place for details.";
