(function () {
  ...
  const SOURCES = freeze({
    ...
    ownerManualVerification: source("owner-manual-verification-2026-09-22", "docs/KMK_OWNER_CONFIRMED_SOURCE_CORRECTIONS_2026-09-22.md", null, "L2", "2026-09-22"),
  ...
  const FACTS = freeze([
    ...
    fact("cafe-admin.hours.source-a", "cafe-admin", "hours", { schedule: freeze({ default: "08:00-15:00" }), label: "08:00-15:00" }, [p("schoolEnvironment", 9)], { timeSensitive: true, requiresReview: true, status: "STALE", semanticKey: "cafe-admin.hours.current", conflicts: ["cafe-admin.hours.source-b"], dimension: "historical-hours" }),
    fact("cafe-admin.hours.source-b", "cafe-admin", "hours", { schedule: freeze({ weekdays: "08:00-16:00", weekends: "closed" }), label: "weekdays 08:00-16:00; weekends closed" }, [p("dormGuide", 15)], { timeSensitive: true, requiresReview: true, status: "STALE", semanticKey: "cafe-admin.hours.current", conflicts: ["cafe-admin.hours.source-a"], dimension: "historical-hours" }),
    fact("cafe-admin.hours.owner-confirmed", "cafe-admin", "hours", { en: "Teachers may enter Cafe Admin from 8:00am. Students may enter after 12:30pm. Cafe Admin closes at 3:00pm for everyone.", ms: "Guru boleh memasuki Cafe Admin mulai 8:00 pagi. Pelajar hanya boleh masuk selepas 12:30 tengah hari. Cafe Admin ditutup pada 3:00 petang untuk semua.", zh: "教师可从上午8:00进入 Cafe Admin。学生只能在中午12:30后进入。Cafe Admin 对所有人统一于下午3:00关闭。" }, [SOURCES.ownerManualVerification], { effectiveFrom: "2026-09-22", timeSensitive: true, requiresReview: true, semanticKey: "cafe-admin.hours.current", dimension: "hours" }),
    fact("cafe-admin.rule.access.owner-confirmed", "cafe-admin", "rule", { en: "Teachers may enter Cafe Admin from 8:00am. Students may enter only after 12:30pm. Cafe Admin closes at 3:00pm for everyone.", ms: "Guru boleh memasuki Cafe Admin mulai 8:00 pagi. Pelajar hanya boleh masuk selepas 12:30 tengah hari. Cafe Admin ditutup pada 3:00 petang untuk semua.", zh: "教师可从上午8:00进入 Cafe Admin。学生只能在中午12:30后进入。Cafe Admin 对所有人统一于下午3:00关闭。" }, [SOURCES.ownerManualVerification], { effectiveFrom: "2026-09-22", timeSensitive: true, requiresReview: true, dimension: "rules" }),
    ...
    fact("basketball.fee.owner-confirmed", "basketball-court", "fee", { en: "The Basketball Court is free to use for all KMK students.", ms: "Gelanggang bola keranjang boleh digunakan secara percuma oleh semua pelajar KMK.", zh: "篮球场供所有 KMK 学生免费使用。" }, [SOURCES.ownerManualVerification], { effectiveFrom: "2026-09-22", timeSensitive: true, requiresReview: true, dimension: "fees" }),
    fact("basketball.hours.owner-confirmed", "basketball-court", "hours", { en: "No fixed closing time is confirmed for the Basketball Court. However, KMK students are not allowed to go out to sports facilities after 7:00pm.", ms: "Tiada waktu tutup tetap yang disahkan untuk gelanggang bola keranjang. Walau bagaimanapun, pelajar KMK tidak dibenarkan keluar ke kemudahan sukan selepas 7:00 malam.", zh: "目前没有确认篮球场固定的关闭时间。不过，KMK 学生晚上7:00后不得外出前往体育设施。" }, [SOURCES.ownerManualVerification], { effectiveFrom: "2026-09-22", timeSensitive: true, requiresReview: true, status: "PARTIAL", semanticKey: "basketball.hours.current", dimension: "hours" }),
    fact("basketball.rule.sports-access-after-19", "basketball-court", "rule", { en: "KMK students are not allowed to go out to sports facilities after 7:00pm.", ms: "Pelajar KMK tidak dibenarkan keluar ke kemudahan sukan selepas 7:00 malam.", zh: "KMK 学生晚上7:00后不得外出前往体育设施。" }, [SOURCES.ownerManualVerification], { effectiveFrom: "2026-09-22", timeSensitive: true, requiresReview: true, dimension: "rules" }),
  ]);
